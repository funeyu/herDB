...
public class FSDirectory {
	 ...
	 * @param delete 目录存在删除为true，不删除为false
	...
	/**
	 * 打开一个目录, 此目录必须是herDB的目录，如果有锁文件就throw exception
	 * 然后再进行枷锁的操作
	 * @param directoryPath 要打开的文件夹
	 * @param onlyRead true:只读不写 false:读写
	 * @return
	 */
	...
	/**
	 * 目录下只有文件，没有二级的文件夹，删除目录里所有的文件
	 * 留着一个空的目录
	 * @param dir 将要删除文件的目录
	 * @return 只要有一个删除不成功就返回false  
	 *         全部删除完成返回true
	 */
	...
	/**
	 * 通过FSDirectory创建FSDataStream，负责目录下所有的数据读写操作
	 * 包括index文件的读取，与rawData数据文件的读取
	 * @param name 该目录下的文件名
	 * @return 
	 * @throws Exception
	 */
	public FSDataStream createDataStream(String name) 
			throws Exception{
	...
	class FSDataStream extends InputOutData{
		...
		public FSDataStream(File dir, String name)
			throws Exception{
		...
		@Override
		public byte[] seek(long offset, int size) 
			throws IOException {
